  ...
  <div class="relative w-full min-h-screen bg-gradient-to-br from-[#050505] via-[#0a0000] to-[#1a0000] px-8 py-16 overflow-hidden">
    ...
    <div class="max-w-7xl mx-auto relative z-10">
      <div class="flex items-center gap-4 mb-16">
        <NuxtLink
          :to="`/blog/categories/${route.params.id}`"
      ...
      <template v-else-if="error">
        <UAlert
          ...
          color="red"
          variant="soft"
          title="Помилка завантаження категорії"
        />
      </template>
...
const { data: category, pending, error } = await useFetch(
  `${config.public.apiBase}/blog/categories/${route.params.id}`,
  { server: false }
)
...
watch(category, (newCategory) => {
  if (newCategory && !formData.title) {
    formData.title = newCategory.title || ''
    formData.slug = newCategory.slug || ''
    formData.parent_id = newCategory.parent_id || null
    formData.description = newCategory.description || ''
    originalSlug.value = newCategory.slug || ''
  }
}, { immediate: true })

const parentCategoryOptions = computed(() => {
  const categories = categoriesData.value?.data || categoriesData.value || []
  const filteredCategories = categories.filter((cat: any) => cat.id !== parseInt(route.params.id as string))
...
const onTitleChange = (event) => {
  ...
  if (newTitle && formData.slug === originalSlug.value) {
...
const saveCategory = async () => {
  ...
  try {
    ...
    await router.push(`/blog/categories/${route.params.id}`)
  } catch (err: any) {
...
useHead({
  title: computed(() => `Редагувати: ${formData.title || 'Завантаження...'}`)
})
</script>
