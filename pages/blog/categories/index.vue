        ...
        <div class="overflow-x-auto">
          ...
          >
            ...
            <template #parent_category.title-data="{ row }">
              <span class="text-gray-300">{{ row.parent_category?.title ?? '—' }}</span>
            </template>
...
const { data: raw, pending: loading, error, refresh } = await useFetch<Category[]>(`${config.public.apiBase}/blog/categories`, {
  server: false
})
...
const handleRowClick = (row: Category) => {
  router.push(`/blog/categories/${row.id}`)
}
...
function getDropdownActions(category: Category) {
  return [
    [{
      ...
      click: () => router.push(`/blog/categories/${category.id}`)
    }, {
      label: 'Редагувати',
      icon: 'i-heroicons-pencil-20-solid',
      click: () => router.push(`/blog/categories/${category.id}/edit`)
    }],
    [{
      label: 'Видалити',
      icon: 'i-heroicons-trash-20-solid',
      click: () => handleDelete(category.id, category.title)
    }]
  ]
}
